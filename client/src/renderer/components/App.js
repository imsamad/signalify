...
const signalify_ID_REGEX = /^[0-9a-fA-F]{40}$/;
const WORDS_REGEX = /\S/;
...
export default class App extends React.Component {
  ...
  composeChatHandler(id) {
    // Validate id
    let [signalifyId] = id.match(signalify_ID_REGEX) || [];
    let [publicKey] = id.match(PUBLIC_KEY_REGEX) || [];

    // Ensure id is either a valid signalifyId or PGP public key
    if (!signalifyId && !publicKey) {
      notifications.show('Invalid signalifyId or PGP key', 'error', true, 3000);
      return;
    }
    ...
    ipcRenderer.send('add-chat', signalifyId, publicKey);
  }
