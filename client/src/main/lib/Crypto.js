...
module.exports = class Crypto {
  ...
  async sign(message) {
    ...
    const cleartextMessage = await pgp.createCleartextMessage({
      text: message,
    });
  ...
  async verify(id, message, signature) {
    ...
    const signatureObj = await pgp.readSignature({
  ...
  async _initKey(passphrase) {
    const { publicKeyArmored, privateKeyArmored } = this._armoredSelfKey;
    const publicKey = await pgp.readKey({ armoredKey: publicKeyArmored });
    ...
    this._selfKey = { publicKey, privateKey };
    // Get user info
    const id = publicKey.getFingerprint();
    const address = parseAddress(publicKey.users[0].userID.userID);
    this._armoredSelfKey.user = { id, ...address };
  ...
  _calcMessageKey(ratchet) {
    ...
    return chunk(
      this._HKDF(messageKey, 'signalifyCrypt', null, MESSAGE_KEY_LEN),
      MESSAGE_CHUNK_LEN
    );
  }
