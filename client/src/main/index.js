...
(async () => {
  ...
  function receiverNotFoundHandler({ type, receiverId }) {
    if (type === 'chat-request') {
      windows.main.send(
        'notify',
        'Recipient not on signalify or is offline',
        'error',
        true,
        4000
      );
  ...
  async function addChatHandler(event, signalifyId, publicKeyArmored) {
    if (!signalifyId) {
      // signalifyId, i.e. the userId, is not given so try to extract it from the
      // given PGP publicKey
      try {
    ...
    signalifyId = signalifyId.toLowerCase();
    // Ensure it hasn't been already added or is own
    if (chats.has(signalifyId) || signalifyId === profile.id) {
